 ...
 * Updated for new evm API with chain presets
 * 
...
const paymentConfig: Record<string, { 
  route: string; 
  extensions: { skale: { encrypted: boolean; confidentialToken: boolean }; gasless: boolean };
  description: string 
}> = {
  transfer: {
    route: '/pay/transfer',
...
async function checkBalances() {
  ...
  if (sFuelBalance === 0n && !config.extensions.gasless) {
  ...
  try {
    ...
    const minBalance = 10n ** BigInt(usdcDecimals - 3)
...
async function main() {
  try {
    ...
    console.log(`   Payment mode: ${config.extensions.skale.encrypted ? 'confidential' : 'normal'}, ${config.extensions.gasless ? 'gasless' : 'user pays gas'}`)
    console.log('')
